...
(assert (forall ((x Lst)) (= (lst-union nil x) x)))
(assert (forall ((n Nat) (x Lst) (y Lst)) (= (lst-union (cons n x) y) (ite (lst-mem n y) (lst-union x y) (cons n (lst-union x y))))))
(assert (not 
(forall ((x Nat) (y Lst) (z Lst)) (=> (lst-mem x y) (lst-mem x (lst-union y z))))  ; G42 
))
(check-sat)
