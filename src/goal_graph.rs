...
impl GoalGraph {
  ...
  pub fn new_lemma(&mut self, root: &GoalInfo, prop: Option<&Prop>) {
    ...
    self
      .lemma_index_map
      .insert(root.lemma_id, LemmaNode::new(root, prop_id));
  }
  pub fn exclude_bid_reachable_lemmas(
    ...
    for (prop, (x, y)) in prop_list.iter().zip(id_list.iter()) {
      ...
      match visited.get_mut(&feature) {
        ...
        Some(pre) => {
          if prop.1.to_string().len() < pre.1.to_string().len() {
            *pre = prop.clone();
          }
        }
      }
  ...
  pub fn relink_related_lemmas(&mut self) {
    ...
    for goal in self.goal_index_map.values_mut() {
      goal.related_lemmas = goal
        .related_lemmas
        .iter()
        .map(|lemma_id| repr_id_map[lemma_id])
        .collect();
    }
  }
      ...
      if let Some(active_lemmas) = raw_active_lemmas {
        ...
        for info in raw_res {
          if self
            ...
            .related_lemmas
            .iter()
            .any(|w| active_lemmas.contains(w))
          {
            res.push(info);
          }
        }
