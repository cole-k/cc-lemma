...
impl GoalNode {
  fn new(goal: &GoalIndex, father: Option<WeakGoalRef>) -> GoalNode {
    GoalNode {
      name: goal.name.clone(),
      lemma_id: goal.lemma_id,
      full_exp: goal.full_exp.clone(),
      father,
      status: GoalNodeStatus::Unknown,
      connect_lemmas: Vec::new(),
      sub_goals: Vec::new(),
    }
  }
}
...
impl GoalGraph {
  ...
  fn propagate_goal_status(node: &RefCell<GoalNode>) {
    ...
    let sub_status: Vec<_> = node
      .borrow()
      .sub_goals
      .iter()
      .map(|sub_node| sub_node.borrow().status)
      .collect();
