...
impl ExpressionStorage {
  fn new(size_limit: usize) -> Self {
    ExpressionStorage {
      exp_storage: vec![HashSet::new(); size_limit + 1],
    }
  }
  ...
  fn construct(size_limit: usize, op: String, sub_storages: &Vec<ExpressionStorage>) -> Self {
    ...
    if sub_storages.is_empty() {
      if size_limit >= 1 {
        res.exp_storage.get_mut(1).unwrap().insert(op);
      }
      return res;
    }
    ...
    for size_list in cartesian_product(&size_pool) {
      ...
      let expression_pool: Vec<Vec<String>> = size_list
        .iter()
        .zip(sub_storages.iter())
        .map(|(size, sub_storage)| sub_storage.exp_storage[*size].clone().into_iter().collect())
        .collect();
...
pub fn print_all_expressions_in_egraph(
  egraph: &EGraph<SymbolLang, CycleggAnalysis>,
  size_limit: usize,
) {
  let mut node_info: HashMap<Id, ExpressionStorage> = egraph
    .classes()
    .map(|class| (class.id, ExpressionStorage::new(size_limit)))
    .collect();
  ...
  while is_changed {
    ...
    for class in egraph.classes() {
      for node in class.nodes.iter() {
        ...
        let sub_storage = node
          .children
          .iter()
          .map(|id| node_info.get(&egraph.find(*id)).unwrap().clone())
          .collect_vec();
        ...
        //}
        let new_storage =
          ExpressionStorage::construct(size_limit, node.op.to_string(), &sub_storage);
  ...
  T: PartialOrd + PartialEq + Clone,
{
  ...
  pub fn insert(&mut self, elem: T) {
    ...
    if num_inserts == 0 {
      ...
      warn!(
        "Inserted an element {} times in the chain set.",
        num_inserts
      );
    }
  }
